...
import { 
  Radio, Swords, Shield, Wand2, FlaskConical, BookOpen, Settings, Backpack, Box, Zap, X,
  LucideIcon, Flame, Droplet, Wind, Lock, Unlock, CheckCircle2, ChevronRight
} from 'lucide-react';
import { useGameStore } from '../store';
import { soundService } from '../services/soundService';

type TabId = 'voice' | 'skills' | 'inventory' | 'items' | 'armor' | 'staffs' | 'magic' | 'potions' | 'spells' | 'settings';

export function QuickAccessBar() {
  const [activeTab, setActiveTab] = useState<TabId | null>(null);
  
  const TABS: { id: TabId; label: string; icon: LucideIcon; color: string }[] = [
    { id: 'voice', label: 'Comms', icon: Radio, color: 'text-cyan-400' },
...
function SkillsPanel() {
  ...
  const toggleSkill = (id: string) => {
    if(equippedSkills.includes(id)) {
      ...
      else soundService.playSFX('ui_click');
    }
    soundService.playSFX('ui_click');
  };
